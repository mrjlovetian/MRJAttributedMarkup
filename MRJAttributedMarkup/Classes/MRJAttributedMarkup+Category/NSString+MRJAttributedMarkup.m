...
NSString *kMRJAttributedMarkupLinkName = @"MRJAttributedMarkupLinkName";
...
- (void)setLink:(NSURL *)url range:(NSRange)range onAttributedString:(NSMutableAttributedString *)as {
    [as removeAttribute:kMRJAttributedMarkupLinkName range:range]; // Work around for Apple leak
    if (url) {
        [as addAttribute:kMRJAttributedMarkupLinkName value:[url absoluteString] range:range];
    }
}
